...
// ✅ CRITICAL: Socket.IO'yu HTTP server'a DOĞRUDAN bağla
console.log('🔧 Initializing WebSocket server...');
const webSocketServer = new WebSocketServer(server);
console.log('✅ WebSocket server initialized and attached to HTTP server');

// ✅ YÖNTEM 1: Socket.IO'yu doğrudan HTTP server'a attach et (middleware olmadan)
const io = webSocketServer.getSocketIOInstance();
app.set('socketio', io);

console.log('✅ Socket.IO directly attached to HTTP server (no Express middleware)');
...
server.listen(PORT, HOST, () => {
  ...
  console.log(`🔌 Socket.IO: http://${HOST}:${PORT}/ws/`);
